...
namespace Pentagonal\Sso\Core\Utils\Generator;
...
class UUID implements Stringable
{
    ...
    public static function integerId(string $uuid): ?string
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            $dec = self::multiplyInt($dec);
            // -> bcadd the decimal by the integer value of the hex char
            $dec = self::add($dec, hexdec($hex[$i]));
        }

        return $dec;
    }

    private static function add($a, $b): string
    {
        static $bc = null;
        if ($bc === null) {
            $bc = function_exists('bcadd');
        }
        $b = strval($b);
        return $bc ? bcadd($a, $b) : BasicMath::create($a)->add($b);
    }

    private static function multiplyInt($a): string
    {
        static $bc = null;
        if ($bc === null) {
            $bc = function_exists('bcmul');
        }
        return $bc
            ? bcmul($a, '16')
            : BasicMath::create($a)->multiply(16);
    }
    ...
    /**
     * @inheritdoc
     * Default string-able return uuid v4
     */
